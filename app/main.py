# app/main.py  – models resident in VRAM, pushes JSON then WAV
import io, json, os, shutil, tempfile, traceback, zipfile
...
def safe_unzip(zip_path: Path, final_dir: Path, url: str) -> None:
    ...
    if final_dir.joinpath("config.json").exists():
        print(f"{final_dir} already present – skip download/unzip")
        return
...
async def _startup():
    ...
    print("🚀 Booting pod – loading models into GPU…")
    load_models()
