...
try:
    ...
    class SnacConfig(dict):
        def __getattr__(self, k):  # pylint: disable=invalid-name
            return self[k]
...
# ─── offline SNAC loader (quiet, strict=False fallback) ────────
def load_snac_local(model_dir: Path, device: str = "cpu") -> SNAC:
    ...
    print(f"✅  SNAC vocoder ready  | cfg: {cfg_file.name} | ckpt: {ckpt.name} (strict=False)")
